...
void EffectComponent::OnCreate()
{
	...
	circleouter->CreateBitmapResource(Singleton<AppPaths>::GetInstance().GetWorkingPath() + L"\\..\\Resource\\Sprites\\Effect\\Parry\\circle_outer.png");

	circleinner = owner->AddComponent<BitmapRenderer>();
	circleinner->SetUseCustomRect(true);
	circleinner->CreateBitmapResource(Singleton<AppPaths>::GetInstance().GetWorkingPath() + L"\\..\\Resource\\Sprites\\Effect\\Parry\\circle_inner.png");

	linehorizon = owner->AddComponent<BitmapRenderer>();
	linehorizon->SetUseCustomRect(true);
	linehorizon->CreateBitmapResource(Singleton<AppPaths>::GetInstance().GetWorkingPath() + L"\\..\\Resource\\Sprites\\Effect\\Parry\\line_horizon.png");
