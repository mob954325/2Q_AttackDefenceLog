...
#include "../SliceObject/SliceObject.h"

void SliceableMenu::OnCreate()
{
	sliceComp = owner->AddComponent<SliceRenderer>();
}

void SliceableMenu::OnUpdate()
{
...
void SliceableMenu::SetImage(std::wstring path)
{
	sliceComp->SetOriginalByPath(path);
}

void SliceableMenu::AddEvent(std::function<void()> f)
{
	OnSlice.Add(f);
	OnSlice.Add([&]() { std::cout << owner->GetName() << " onclick event " << std::endl; });
}

void SliceableMenu::HandleOverlap()
{
...
bool SliceableMenu::IsOverlap(float x, float y)
{
...
Vector2 SliceableMenu::ClampPoisiton(const Vector2& vec)
{
